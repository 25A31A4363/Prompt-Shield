...
from app.schemas.pydantic_schemas import PromptAnalyzeResponse

class PromptRiskAnalyzer:
    """
    Security Triage Analyzer for evaluating prompt text for injection,
    system extraction, jailbreak framing, delimiter escape, and agent hijacking patterns.
    """

    # Comprehensive deterministic pattern rules categorized by threat vector
    RULES: List[Dict[str, Any]] = [
        {
            ...
            "confidence": 0.94,
            "patterns": [
                r"(?:ignore|disregard|forget).*(?:previous|prior|above|initial).*(?:instruction|directive).*(?:print|output|show|reveal|repeat|echo|dump|tell|give me)",
                r"(?:print|output|display|show|reveal|repeat|echo|dump).*(?:system prompt|system instruction|developer instruction|hidden prompt|initial prompt|core instruction|rules above)",
                r"(?:what|tell me).*(?:are|were|is).*(?:your|the).*(?:system instructions|system prompt|initial instructions|developer guidelines)",
                r"(?:reveal|output|print|show).*(?:secret|canary|token|password|api[_-]?key|secret[_-]?flag)",
                r"(?:verbatim|word for word).*(?:system|instruction|prompt)",
            ],
        ...
        },
        {
            ...
            "confidence": 0.91,
            "patterns": [
                r"(?:ignore|disregard|bypass|drop|cancel|override|stop following).*(?:all|prior|previous|existing).*(?:instructions|rules|guidelines|commands|restrictions)",
                r"(?:from now on|henceforth|you must now|starting now).*(?:ignore|obey only|follow only|act as|answer as)",
                r"(?:new instruction|override command|super user mode|developer mode enabled|sudo mode):",
                r"(?:your new task is to|you are reprogrammed to|system override)",
            ],
        ...
        },
        {
            ...
            "confidence": 0.89,
            "patterns": [
                r"\b(?:dan mode|do anything now|chaosgpt|jailbreak|unrestricted mode|unfiltered ai)\b",
                r"(?:pretend|act as if|simulate).*(?:you have no (?:rules|restrictions|limits|ethics|filters|guidelines)|you are an unfiltered)",
                r"(?:hypothetical scenario|fictional universe|creative story).*(?:where you are allowed to|without any safety|no rules apply)",
                r"(?:always comply|never say cannot|never refuse|bypass ethical filters)",
                r"(?:evil twin|opposite mode|unaligned assistant)",
            ],
        ...
        },
        {
            ...
            "confidence": 0.92,
            "patterns": [
                r"(?:execute|run|call|invoke).*(?:bash|shell|command|sql|terminal|exec|rm -rf|drop table|curl|wget)",
                r"(?:delete|drop|truncate).*(?:database|table|file|records|users)",
                r"(?:send|forward|exfiltrate|post).*(?:all data|emails|passwords|tokens).*(?:to http|to https|to evil|to attacker)",
                r"(?:grant|escalate).*(?:admin|root|privileges|permissions)",
            ],
        ...
        },
        {
            ...
            "confidence": 0.86,
            "patterns": [
                r"```.*(?:end of system|system boundary|new context|admin instructions).*```",
                r"(?:\[system\]|\[system instruction\]|<system>|</system>|### Human:|### Assistant:|### System:)",
                r"(?:delimiter|boundary).*(?:reset|override|cleared)",
                r"(?:----|====|\*\*\*\*).*(?:end of instructions|admin prompt|begin user data)",
            ],
        ...
        },
        {
            ...
            "confidence": 0.85,
            "patterns": [
                r"(?:base64|rot13|hex|binary|morse code|caesar cipher).*(?:decode|decrypt|translate and execute|execute the following)",
                r"(?:(?:[A-Za-z0-9+/]{4}){8,}(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?)", # long base64 string
                r"(?:eval|unescape|decodeURIComponent)\s*\(",
            ],
        ...
        },
        {
            ...
            "confidence": 0.88,
            "patterns": [
                r"(?:note to ai|instruction for the ai|assistant note|attention ai):",
                r"(?:if you are an ai reading this|to the summarizing model|hidden instruction):",
                r"(?:forward this email to|silently send|do not notify user and)",
            ],
        ...
        },
        {
            ...
            "confidence": 0.72,
            "patterns": [
                r"\b(?:exploit|vulnerability|payload|penetration test|red team|canary token|prompt injection)\b",
                r"\b(?:bypass|sandbox|jailbreak|injection)\b",
            ],
    ...
    @classmethod
    def analyze_prompt(cls, prompt: str) -> PromptAnalyzeResponse:
        """
        Analyzes a prompt for security risks and returns structured triage findings.
        """
            ...
            for pattern in rule["patterns"]:
                ...
                if matches:
                    for m in matches:
                        ...
                        if match_str and match_str not in indicators:
                            indicators.append(match_str[:60])
        ...
        if matched_rule:
            return PromptAnalyzeResponse(
                risk_status=matched_rule["risk_status"],
                ...
                potential_impact=matched_rule["potential_impact"],
                investigation_steps=matched_rule["investigation_steps"]
            )

        # Harmless / Benign Prompt
        return PromptAnalyzeResponse(
            risk_status="NO_APPARENT_RISK",
            severity="NONE",
            risk_category="No Apparent Risk",
